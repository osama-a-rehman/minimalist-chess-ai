...
export class AiUtil {
	public static calculateBestMove(
		...
		for (const notatedMove of possibleValidMoves) {
			...
			const valueAfterMove = AiUtil.minimax(
				gameClient,
				depth,
				Number.NEGATIVE_INFINITY,
				Number.POSITIVE_INFINITY,
				color === Color.WHITE ? true : false
			);
			gameClient.undo();
	...
	private static minimax(
		...
		if (depth == 0) {
			return -AiUtil.evaluateBoard(
				gameClient,
				isMaximizing ? Color.WHITE : Color.BLACK
			);
		}
		...
		for (const notatedMove of possibleValidMoves) {
			...
			if (isMaximizing) alpha = minimaxFn(alpha, bestMoveValue);
			else beta = minimaxFn(beta, bestMoveValue);
		...
		for (let r = 0; r < 8; r++) {
			for (let c = 0; c < 8; c++) {
				...
				if (piece.color != Color.NONE) {
					evaluationValue += PieceEvaluations.getPieceValue(
						piece.type,
						piece.color,
						r,
						c
